...
# Scrape Google Trends one topic/term × national geo=MX at a time (independent 0-100).
#
# Usage (from repo root):
#   Rscript day3-0900-coding-exercise/scripts/01_download_gtrends_topics_national.R
#   Rscript day3-0900-coding-exercise/scripts/01_download_gtrends_topics_national.R --max-queries 5
#
...
source("day3-0900-coding-exercise/scripts/functions/gt_topic_adm1_helpers.R")
...
# Same set as 01_download_gtrends_topics_adm1.R
topic_queries <- c(
  "Aedes",
  "Aedes aegypti",
  "mosquito",
...
dl_root <- "day3-0900-coding-exercise/downloads"
...
if (length(existing_dirs) && any(csv_counts > 0L)) {
  ...
  out_dir <- file.path(dl_root, as.character(Sys.Date()))
}
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
meta_path <- file.path(out_dir, "topic_resolution_log_national.csv")
meta_rows <- list()
...
message(
  "National (geo=MX) sequential download | sleep=", sleep_sec, "s",
...
for (qi in seq_len(nrow(jobs))) {
  ...
  fname <- sprintf("gtrends_%s_%s.csv", canon, geo)
  ...
  if (is.null(series) || nrow(series) == 0L) {
    # Persist failure so resume does not re-hammer Google for empty term pairs.
    writeLines(
      c(
        paste0("query=", q),
        paste0("geo=", geo),
