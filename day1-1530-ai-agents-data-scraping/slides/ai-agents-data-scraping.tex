...
\begin{frame}{Google Trends has no official API}
  Google Trends is itself the messy case: no official API, an internal endpoint that keeps
  changing, aggressive rate limits, and the sampling instability from this afternoon.
  \begin{itemize}
    \item \texttt{pytrends} scrapes that endpoint for you; when it breaks, the agent reads
          the error, adjusts, and retries, instead of you fighting it.
    \item You describe the \emph{outcome} (terms, geo, window, retry-on-429); the agent
          writes the plumbing and hands you a tidy CSV.
  \end{itemize}
  \vspace{0.3em}
  \begin{block}{The shift}
    Old way: fight an unofficial API for an afternoon. New way: a paragraph, then you are
    checking a plot. The hard part becomes \textbf{judgment}, not plumbing.
  \end{block}
\end{frame}
